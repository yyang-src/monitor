...
analyzer_ip=ARGV[2] || nil
puts "Loading #{filename}.... for port #{server_port} and analyzer #{analyzer_ip||'ALL'}"
...
for p in network
   # print packet data for each packet that matches the filter
     packet_counter +=1
   if p.tcp? && (p.sport.to_i == server_port.to_i || p.dport.to_i==server_port.to_i) && (analyzer_ip.nil? || p.src.to_s == analyzer_ip || p.dst.to_s == analyzer_ip)
      direction= (p.dport.to_i == server_port.to_i ? "TO ANALYZER |#{p.dst}|" : "FROM ANALYZER |#{p.src}|")
      puts "#{packet_counter} => #{direction } @ #{p.time} "
      if p.tcp_data.nil?
        next
      end
