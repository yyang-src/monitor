   ...
   while ( input != 'Q' )
      begin
         if (input == 'SL')
            ...
            dt=instrument.set_date_time(2010,3,26,7,38,8)
            puts dt.inspect()
         elsif (input == 'SS')
            ...
            settings=instrument.set_settings({"central_freq"=>freq.to_i, "display_type"=>3,
	    ...
	    puts "===========================DRUM-ROLL PLEASE!==============================="
            t1 = Time.now
            response=instrument.carrier_offset()
            t2 = Time.now
            puts t2-t1
            ...
            puts "Frequency set to #{freq}"
            t1 = Time.now
            response=instrument.digital_trigger(5)
            t2 = Time.now
            puts "speed time = #{t2 - t1}"
            puts "TRIGGER:"
            puts response.inspect()
            #instrument.set_mode(11)
            #response=instrument.get_modulation()
