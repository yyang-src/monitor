...
#    txt=analyzer.cmd('String'=>'rm /usr/local/bin/atsoft/monitor/*', 'Match'=>/\/usr\/local\/bin\/atsoft #/)
  ...
  def InstrumentUtils.snmp_alarm(manager, seq, alarm_type, alarm_lvl, utc_time,ip, descr="", hardware="", 
  ...
  def InstrumentUtils.snmp_alarm_lvk(trap_type,manager, seq, site_name,port_name,start_time,ip,channel="",threshold="",measure="", current_value="", alarm_id="",alarm_type="",alarm_lvl="", descr="",trace_url="")
  ...
  def InstrumentUtils.snmp_monitoring(trap_type, manager, seq, anl_id, anl_name, try_count, monitor_type, descr,ip)
    var_binds= [
      SNMP::VarBind.new("1.3.6.1.4.1.10300.1.1.3.1",SNMP::Integer.new(seq)),
      SNMP::VarBind.new("1.3.6.1.4.1.10300.1.1.3.2",SNMP::Integer.new(anl_id)),
      SNMP::VarBind.new("1.3.6.1.4.1.10300.1.1.3.3",SNMP::OctetString.new(anl_name)),
      SNMP::VarBind.new("1.3.6.1.4.1.10300.1.1.3.4",SNMP::Integer.new(try_count)),
      SNMP::VarBind.new("1.3.6.1.4.1.10300.1.1.3.5",SNMP::Integer.new(monitor_type)),
      SNMP::VarBind.new("1.3.6.1.4.1.10300.1.1.3.6",SNMP::OctetString.new(descr)) 
     ]
    SNMP::Manager.open(:Host=>manager, :Version => :SNMPv1) do |snmp|
      $logger.debug(ip) if !$logger.nil?
      snmp.trap_v1(
        "enterprises.10300.1.1.6",
        ip,
        :enterpriseSpecific,
  ...
  def InstrumentUtils.snmp_restore(manager,seq, alarm_type, utc_time, ip)
    if !InstrumentUtils.validate_alarm_type(alarm_type)
      raise SNMP_Error.new("Alarm type [#{alarm_type}] not valid")
    end
    var_binds= [
