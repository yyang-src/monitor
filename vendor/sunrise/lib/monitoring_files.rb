module MonitorFiles
   ...
   class SignalsFO < FileObj
      ...
      def SignalsFO::build(analyzer_id, obj_name='CSignalSource')
         signal_list=[]
         analyzer_obj=Analyzer.find(analyzer_id)
         switches=analyzer_obj.active_switches
...
#pulled from the presets.h file. rever to SAStatus structure
      ...
      def TraceReferenceFO::build(analyzer_id, obj_name='CTraceReference')
         ...
         anl.active_switches.each {|swt|
            swt.switch_ports.each { |swp|
               ...
               raise SunriseError.new("Profile Frequency Range is nil !") if selected_profile.start_freq.nil? or selected_profile.stop_freq.nil?
               ...
               trace=Datalog.map_data(selected_profile.start_freq, selected_profile.stop_freq,
   ...
   class SwitchesFO < FileObj
      ...
      def SwitchesFO::build(analyzer_id, obj_name='CSwitchNetwork')
         ...
         switch_count=dbrec.active_switches.count()
         ...
         rptp_list=[]
         idx=0
         switch_count=dbrec.active_switches.length
         dbrec.active_switches.each { |switch|
            idx=idx+1
            if (dbrec.switch_type == 3) #Handling Electroline switches.
               if ((idx % 4) == 1)
                  switch_list.push({:switch_name=>switch[:switch_name], :port_nbr=> switch[:address], :port_count=>switch_count*16})
               end
            else
               switch_list.push(
      ...
      def SwitchesFO::specRead(f, obj_name)
         ...
         puts switch_list.inspect()
         switch_net.switch_list=switch_list
         rptp_count.times { |rptp_idx|
            rec_data_str=f.read(41)
            raise("Switch data not right @ #{rptp_idx} => #{rec_data_str.inspect()} and existing data loooks like \r\n #{rptp_list.inspect()}") if (rec_data_str.nil? || rec_data_str.length!= 41)
