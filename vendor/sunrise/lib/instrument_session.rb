...
class InstrumentSession
   attr_reader :ip,:port,:rw_addr,:analyzer_addr,:dl_process
   attr_writer :socket,   :todo_list, :dl_process, :logger,:command_process
   attr_accessor :dir_prefix,:nack_error
   attr_accessor :command_record
   ...
   QamQuery=3
   AnalogQuery=4
   def initialize(ip,port,rw_addr, logger=nil,analyzer_addr='01', analyzer_id=nil,is_debug = false)
   ...
   def read_data(size, timeout=10)
       result = ''
       while size > 0
           return if select([@socket], nil, nil, timeout).nil?
           received = @socket.recv(size)
           if (received.nil? || received.empty?)
               @logger.debug "received nothing in instrument_session.read_data"
               raise SunriseError.new("Socket Closed unexpectedly.")
           end
   ...
   def read_response(timeout=10)
       ...
       #puts "  start read_response"
       r = read_response_(timeout)
       #puts "  end read_response"

       #@call_num -= 1
       return r
   end

   def read_response_(timeout=10)

       header = read_data(15, timeout)
       if (header.nil? || header.empty?)
           @logger.debug('read_response: No header response.')if timeout != 0
           return nil
       end
       if header[0] != 1 || header[1] != 2
           #puts header.inspect
           @logger.debug('read_response: Invalid packet format.')
           clear_read_buffer
           return nil
           #raise ProtocolError.new("read_response: Invalid packet format.")
       end
       type = header[6, 2].to_i
       len  = header[12,3].to_i
       len  = ((header[12] << 16) | (header[13] << 8) | header[14]) if type == 14

       body = read_data(len + 4, timeout) # 4 extra tail bytes
       if (body.nil? || body.empty?)
           @logger.debug("read_response: Failed to receive message body, type is #{type}.")
           return nil
       end
   ...
   def check_async_messages(avantron_rec)
      if ((avantron_rec.msg_type.to_i == 20) && avantron_rec.msg_object.key?('subcommand') &&
         ...
         ack_async()
         return true
     elsif ((avantron_rec.msg_type.to_i == 90)  && (avantron_rec.msg_object['subcommand'].to_i()==50))
       raise ProtocolError.new(" Switch #{avantron_rec.msg_object['switch_idx']} cannot be accessed.")
      else
         return false
      end
   end
   ...
   def parse_response(timeout=10, expected_msg_type=nil)
      ...
      msg = "time:#{Time.now.sec},response:#{response.msg_obj().inspect}"
      if @is_debug
          @command_record[@record_index]= msg
          @record_index += 1
          @record_index = 0 if @record_index > 20
      end
      return response
   end
   ...
   def send_ack()
         ...
         @logger.debug("Let's send an ACK")
         avatronrec=Avantron::AvantronRec.new(@rw_addr,@analyzer_addr,'01','001',@logger,1)
   ...
   def msg_with_ack(msg_type, node_nbr, msg_obj,packet_seq=1, retry_count=3)
       #puts "begin msg_ack type = #{msg_type}"
       r = msg_with_ack_(msg_type, node_nbr, msg_obj,packet_seq, retry_count)
       #puts "end msg_ack"
       return r
   end

   def msg_with_ack_(msg_type, node_nbr, msg_obj,packet_seq=1, retry_count=3)
      @nack_error=nil
      if @socket.nil?
         raise(ProtocolError.new("Socket has not been set"))
      end
      if @is_debug
          msg = "msg_type=#{msg_type}, node_nbr=#{node_nbr},msg_obj=#{msg_obj.inspect},packet_seq=#{packet_seq}, retry_count=#{retry_count}"
          @command_record[@record_index] = msg
          @record_index += 1
          @record_index = 0 if @record_index > 20
      end
      ...
      while(!got_response && (try_count <= retry_count))
         ...
         #@logger.debug("while looking for my ACK ")
         while(response_packet=read_response(25))
            #@logger.debug("Looking for my ACK ")
            response=AvantronRec.parsePacket(response_packet,true,@logger)
            ...
            if (response.msg_type == '01')
               #@logger.debug("I received a ACK with response: #{response.msg_obj['retcode']} ")
               got_response=true
               return 1
         ...
         @logger.debug("done while looking for my ACK ")
      end
   ...
   def initialize_socket(clear_buffer=true)
      ...
      begin
         ...
         @logger.debug "#{ex.message},#{ex.backtrace}"
      ...
      @socket.setsockopt(Socket::IPPROTO_TCP, 4, 5)  #TCP_KEEPIDLE
      @socket.setsockopt(Socket::IPPROTO_TCP, 5, 5)  #TCP_KEEPINTVL
   ...
   def upload_file(srcfile,destfile, &status_block)
      @logger.debug "begin upload_file in Instrument_session"
      ...
      if response.nil?
          @logger.debug "upload fileblock response is nil"
          return false
      end
      block_size=response.msg_obj()['block_size']
      if (block_size == 0)
        @logger.debug "Failure to Transfer file. Block size is zero"
        raise(ProtocolError.new('Failure to Transfer file.  Block size is zero'))
      end
      #--------------------------
      ...
      open(srcfile, 'r') { |f|
         while (content=f.read(block_size))
            ...
            if (self.msg_with_ack('20','001',{'len'=>0,'subcommand'=>10,'resv1'=>0,
            'seq'=>block_count, 'resv2'=>0,'position'=>block_count,'block_size'=>content.length,'data'=>content},1)!=1)
                @logger.debug "Failed to send content in instrument_session.upload_file"
                raise(ProtocolError.new("Failed to send content"))
            end
      ...
      ack_packet=ack.buildPacket({'retcode'=>0})
      @socket.write(ack_packet)
      @logger.debug "end upload_file in Instrument_session"
      return true
      #--------------
   ...
   def download_file(file, dir_prefix=nil)
      ...
      response=parse_response()
      if response.nil?
          @logger.debug "Download file info response is nil"
          return
      end
      #--------------------------
      # Response for first set of bytes.
      ...
      until byte_count >= file_length
         ...
         if fileblock_obj.nil?
            @logger.debug "Download fileblock response is nil"
            return
         end
   ...
   def diagnostic(timeout=6)
      raise(ProtocolError.new("Failed to Start_diagnostic")) if (self.msg_with_ack('7','001',
         { :hdr_len=>30, :subcommand=>3, :resv=>16, :power_flag=>1,
           :mer_flag=>1, :enm_flag=>1, :evm_flag=>1, :sys_noise_flag=>1,
           :cw_infc_flag=>1, :freq_resp_flag=>1, :margin_flag=>1, :margin_tap_flag=>1,
           :compression_flag=>1, :digital_hum_flag=>1, :phase_noise_flag=>1,
           :IQ_gain_diff_flag=>1, :iq_phase_diff_flag=>1, :carrier_flag=>1, :symbol_rate_flag=>1,
           :group_delay_flag=>1, :equalizer_stress_flag=>1, :micro_reflection_flag=>1, :resv2=>''},2)!= 1)
      giveup=Time.now()+60
	sleep 20
        diagnostic_response=parse_response()
      ...
      return diagnostic_response.msg_obj()
   end

   def carrier_offset(timeout=6)
      raise(ProtocolError.new("Failed to Start_diagnostic")) if (self.msg_with_ack('7','001',
         { :hdr_len=>30, :subcommand=>3, :resv=>16, :power_flag=>0,
           :mer_flag=>0, :enm_flag=>0, :evm_flag=>0, :sys_noise_flag=>0,
           :cw_infc_flag=>0, :freq_resp_flag=>0, :margin_flag=>0, :margin_tap_flag=>0,
           :compression_flag=>0, :digital_hum_flag=>0, :phase_noise_flag=>0,
           :IQ_gain_diff_flag=>0, :iq_phase_diff_flag=>0, :carrier_flag=>1, :symbol_rate_flag=>0,
           :group_delay_flag=>0, :equalizer_stress_flag=>0, :micro_reflection_flag=>0, :resv2=>''},2)!= 1)
      giveup=Time.now()+60
      sleep 5
      diagnostic_response=parse_response()
      if diagnostic_response.nil?
       ...
       diagnostic_response.msg_obj()
   end

   def trigger(acquisition_count=1, node_nbr=nil)
      if (node_nbr.nil?)
         node_nbr="001"
      end
      #'98'=> { :TypeLabel=>'TRIGGER', :Format=>'CC',:Keys=>['trace_count_to_perform','acquisition_buffer_reset']},
      raise(ProtocolError.new("Failed to Trigger")) if (self.msg_with_ack('98',node_nbr,
   ...
   def get_rptp_count
      ...
      if rptp_count_object.nil?
        @logger.debug "rptp response is nil"
        return nil
      end
   ...
   def monitor_client(monitor_period=3)
      ...
      while (Time.now()-time_marker)< monitor_period
         ...
         while ( stat_count > 0 )
            ...
            if response.nil?
                @logger.debug "stat response is nil"
                continue
            end
         ...
         while ( alarm_count > 0 )
            ...
            #TODO do something with the msg_obj
            if response.nil?
                @logger.debug "alarm response is nil"
                continue
            end
            msg_obj=response.msg_obj()
   ...
   def set_mode(mode)
      if (self.msg_with_ack('94','001',{'desiredmode'=>mode},1)!=1)
         raise(ProtocolError.new("Sorry Mode #{mode} must not be supported!"))
   ...
   def set_switch(switch)
      if (self.msg_with_ack('80','001',{'len_msg'=>0,'rptp_number'=>switch},1)!=1)
         ...
         raise(ProtocolError.new("Sorry Switch #{switch} cannot be changed to"))
	 return false
      end
		 return true
   end
   ...
   def ccn_trigger()
      raise(ProtocolError.new("Failed to Trigger")) if (self.msg_with_ack('20','001',{'subcommand'=>10,:resv=>0  },1)!=1)
      response=parse_response()
      if (response.nil?)
         @logger.debug "Trigger Response is nil"
         return nil
   ...
   def get_error_switch
     response=parse_response()
     @logger.debug "Getting Switch Error!"
     return nil if response.nil?
     if ((response.msg_type.to_i != 90) || (response.msg_obj()['subcommand'].to_i()!=50))
       raise ProtocolError.new(
            "Message type '#{response.msg_type.to_s}' is not valid")
     end
     return response.msg_object()
   ...
   def get_hardware_info
      raise(ProtocolError.new("Failed to Get Hardware Info")) if (self.msg_with_ack('20','002',{'len'=>0,'subcommand'=>7, 'resv1'=>0},0)!=1)
   ...
   def set_date_time(yr,month,day,hours,minutes,seconds)
      #['len','subcommand','resv1','year','month','day','hours','minutes','seconds','hunsecs'],
      raise(ProtocolError.new("Failed to Set Date Time")) if (self.msg_with_ack('20','002',
            {'len'=>0,'subcommand'=>6, 'resv1'=>0,'year'=>yr,'month'=>month,'day'=>day,
                'hours'=>hours,'minutes'=>minutes,'seconds'=>seconds,'hunsecs'=>0},0)!=1)
   end
   def get_firmware_version
      raise(ProtocolError.new("Failed to Get Firmware Version")) if (self.msg_with_ack('20','002',{'len'=>0,'subcommand'=>4, 'resv1'=>0},0)!=1)
      response=parse_response(10,20)
      if ((response.msg_type.to_i == 20) && (response.msg_obj()['subcommand'].to_i()==4))
         return response.msg_object()
      end
   end
   def get_settings
