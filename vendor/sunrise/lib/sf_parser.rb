...
def SFParser::system_file_load(filename, display_name=nil)
      ...
      if block_type == 65535
        ...
        output << "reading Block<br>"
         ...
         if (block_type == 0) #System File
            ...
            sf_system_file=get_system_file(display_name)
            current_state[:system_file_id]=sf_system_file.id
  ...
  # support for old system file format , add default test type to every test plan .
  @sf_test_plans.each { |sf_test_plan|
    sf_system_test = SfSystemTest.find(:all, :conditions=> ["sf_test_plan_id = ? and test_type = ?",sf_test_plan.id,Measure::FREQ_ERROR])
    if sf_system_test.empty?
      SfSystemTest.create(:test_type => Measure::FREQ_ERROR, :sf_test_plan_id => sf_test_plan.id, :enable_flag => true,
        :data_type => 0, :max_value => 5, :tolerance => 0, :units => 0)
    end
  }
...
def SFParser::get_system_file(display_name)
