...
# The AvantronRec represents a Avantron Protocol packet that has been recieved or sent.
   class AvantronRec
      ...
      #===build_noise_interfer
      # <b>Build Noise Interference Message </b>
      # Build Request for noise interference message
      def build_noise_interfer()
         cmd={
                :Format=>"SCCCCCCCCCC",
                :Keys => [:len,:subcommand, :resv, :command_state, :measure_state, 
                             :ccn_fld_nbr, :ccn_line_nbr, :cso_fld_nbr, :cso_line_nbr, :ctb_fld_nbr, :ctb_line_nbr ]
         }
         pack(cmd)
      end
      #===noise_interfer
      # <b>Noise Interference Message </b>
      # Parse noise interference message
      def noise_interfer()
         if (@from_analyzer)
            cmd={ :Format => "SCCCCSDDDslDDslDDs", 
                  :Keys => [:len, :subcommand,:resv,:done_flag, :resv2,:resv3, :carr_lvl, :ctn_ratio, :ctn_noise_corr, :ctn_marker, 
                            :cso_freq_off, :cso_ratio, :cso_noise_corr, :cso_marker, :ctb_freq_off, :ctb_ratio, :ctb_noise_corr,:ctb_marker]
                }
         else
            cmd={ :Format => "SCCCCCCCCCC", 
                  :Keys => [:len, :subcommand, :resv, :cmd_state, :meas_to_perform, :ccn_field_nbr, :ccn_line_nbr, :cso_field_nbr, 
                            :cso_line_nbr, :ctb_field_nbr, :ctb_line_nbr]
                }
         end
         return unpack(cmd)
      end
      ...
      def parse_constellation
          #"SCCSL"+"FFSSS"+"LLLL"+"CCCC"
          #value_list=@msg.unpack(QAM_PREFIX_FORMAT+"Sa*")
          #puts value_list.inspect

          length = @msg[44, 2].unpack("S")[0].to_i * 2
          points = @msg[46, length] #.unpack("a*")
          @msg_object = { :point_count => length, :points => points }
          prefix = @msg[0, 44].unpack(QAM_PREFIX_FORMAT)

          @@qam_prefix_keys.each_with_index { |k, i|
              @msg_object[k] = prefix[i]
          }
      end
      # ===general_label
      # <b>Private Function</b>
      ...
      @@msg_types = {
         ...
         '13'=> { :TypeLabel=>'NI', :ParseMsg=>lambda {|obj| obj.noise_interfer()},:BuildMsg=>lambda{|obj| obj.build_noise_interfer()}
         },

         '14'=> { :TypeLabel=>'RAW_SPECTRUM', :Format=>'Ca7a6250', :Keys=>['client','resv1','aligned_image']},
      ...
      @@qam_commands =
      {
         "0"=> {
            ...
            {
               :Label=>"Constellation",
               :ParseMsg=>lambda {|obj| obj.parse_constellation()}
         ...
         "3"=> {
            :FromClient =>
            {
               ...
               :Format=>'SCCCCCCCCCCCCCCCCCCCCCa7',
              #:Format=>'SCCCCCCCCCCCCCCCCCCa10',
               :Keys=>[:hdr_len, :subcommand, :resv, :power_flag, :mer_flag, :enm_flag, :evm_flag, :sys_noise_flag, :cw_infc_flag, :freq_resp_flag,
				  ...
				  :carrier_flag, :symbol_rate_flag, :group_delay_flag, :equalizer_stress_flag, :micro_reflection_flag, :resv2]
            },
            :FromServer=>
            {
               :Label=>"DIAGNOSTIC",
               :Format=>QAM_PREFIX_FORMAT + 'CCSLLLLLL' +'LLLLLLL' + 'lLLLLLS128C17CLC3'+'SSSSC',
               :Keys=>@@qam_prefix_keys + [:resv, :diag_error, :power_level, :mer2, :enm2, :evm2, :sys_noise, :cw_infc, :freq_resp,
                  :echo_margin, :echo_margin_tap_idx, :compression, :digital_hum, :phase_noise, :IQ_gain_diff, :iq_phase_diff,
			:carrier_offset, :symbol_rate_ppm, :ber_pre, :ber_post, :group_delay, :equalizer_stress, :limits, :flags,:postber_flag,:timestamp, :flags2,
			:microrefl05,:microrefl10,:microrefl15, :microrefl20,:resv2]
            }
         }
      }
      # ===@@@remote_commands
      ...
      @@setting_recs= {
         "SA"=> {
            ...
            :Keys => ["msghdr_len","subcommand",'resv1',"resv2",
               "actual_mode","resv3","detector","external_temp","attenuator","internal_freq",
               "central_freq","span","zero_span","video_chan","audio_chan","sweep_time",
               "horizontal_time","resolution_bw","video_bw","avg_length",
         ...
         "7"=> {
            ...
            :FromServer => {
               ...
               :Keys   => ['len','subcommand','resv1','system_type','system_sn','system_cal_date','m20_type','m20_sn',
                  'm20_cal_date', 'm13','m14','m15','m17','m24','m25','m26','m27','m29','m30','eth0',
               'ser','qam_cardtype','qam_cardver','qam_sn','arch','vid','batt','chg','max_freq','qam_mod','resv2'],
               :Format => 'vCCa20a20a20vv'+'a20a24a24a24a24a24a24a24a24a24a24a24'+'a24vva20a24a24a24a24CVa236',
      ...
      def AvantronRec.parsePacket(avantron_packet,from_analyzer,logger)
         ...
         msg_type = header_arr[4].to_i
         msg_len  = header_arr[7].to_i

         if msg_type == 14
             str = header_arr[7]
             msg_len = ((str[0] << 16) | (str[1] << 8) | str[2])
         end

         #puts "PasePackate, message type = #{msg_type}, length = #{msg_len} len_str = #{header_arr[7]}"

         av = AvantronRec.new(header_arr[2], header_arr[3], header_arr[4], header_arr[5], logger,
                              header_arr[6].to_i, msg_len, from_analyzer)
