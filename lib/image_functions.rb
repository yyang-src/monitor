...
module ImageFunctions
   ...
   def write_downstream_trace(attribute, data)
      if data.nil?
         write_attribute(attribute,"")
      else
         data.compact!
         save_data = data.collect{|x|(x*100).round}
         write_attribute(attribute,save_data.pack("s*"))
      end
   end
   def read_downstream_trace(attribute)
      image=read_attribute(attribute)
      if image.nil?
         return nil
      else
          date = read_attribute(attribute).unpack("s*")
          ret_date = date.collect{|x|(x).to_f/100}
         return ret_date
      end
   end
