...
//  fprintf(stderr,"INTERPOLATE START %d - %d -> %d - %d\n",data_start, data_stop, dest_start, dest_stop);
  for (result_pos=0;result_pos < dest_len;result_pos++)
  {
    ...
    {
       //fprintf(stderr,"RYAN: result_freq_pos is out of area\n");
       result[result_pos]=default_val;
       bitarray[result_pos]=0;
    }
    else
    {
       //fprintf(stderr,"RYAN: result_freq_pos is in the area\n");
...
//    fprintf(stderr, "RYAN:result[%d]=%f, bitarray[%d]=%d\n", result_pos, result[result_pos], result_pos, bitarray[result_pos]);
...
void xxx_add(UDF_INIT* initid, UDF_ARGS* args,float(*interpolate_func)(float,float,float),float(*add_func)(float,float,float),float default_val)
{
  ...
  float *interpolated_data = (float *)(initid->ptr+sizeof(struct array_data));
  int *interpolated_bitarray = (int *)(initid->ptr+sizeof(struct array_data)+sizeof(float)*MAXARRAY);
  ...
  if (arr == NULL)
  {
    fprintf(stderr, "Data is NULL,ignored\n");
    return;
  }
  int  array_size=0;
...
//  int *interpolated_bitarray;
  ...
  //fprintf(stderr,"RYAN:array_size=%d\n",array_size);
...
my_bool xxx_init(UDF_INIT *initid, UDF_ARGS *args, char *message, char *sql_func_name)
{
  ...
  if (!(data=(struct array_data *) malloc(sizeof(struct array_data) + sizeof(float)*MAXARRAY + sizeof(int)*MAXARRAY))) /* Freed in deinit*/
  {
  ...
  initid->max_length=8*MAXARRAY*sizeof(float);

//  initid->ptr=(void *)data;
//  initid->max_length=4*MAXARRAY*sizeof(float);

  //fprintf (stderr, "RYAN: xxx_init finish\n" );
  return 0;
}
...
char *maxarray(UDF_INIT *initid, UDF_ARGS *args, char *result,
	       unsigned long *length, char *is_null, char *error)
{
    //fprintf(stderr, "RYAN: maxarray start\n");
    struct array_data* data	= (struct array_data*)initid->ptr;
    *is_null =0;
    *length=MAXARRAY*sizeof(float);
    //fprintf(stderr, "RYAN: maxarray finish\n");
    return (char *)data->arr;
}
/*AvgArray functions*/
...
char *avgarray(UDF_INIT *initid, UDF_ARGS *args, char *result,
	       unsigned long *length, char *is_null, char *error)
{
    //fprintf(stderr, "RYAN: avgarray start\n");
    int i=0;
    int arraylen=0;
    struct array_data* data;
   ...
   *length=MAXARRAY*sizeof(float);
    //fprintf(stderr, "RYAN: avgarray finish\n");
    return (char *)data->arr;
}
